...
def process_wav_files(model, directory, output_directory):
    ...
    for filename in os.listdir(directory):
        if filename.endswith(".wav"):
            ...
            if not os.path.exists(output_path):
                text = recognize_speech(model, wav_path)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process WAV files and generate text using Whisper.")

    parser.add_argument(
        "--wav-directory", default="wav", help="Path to the directory containing WAV files."
    )
    parser.add_argument(
        "--output-directory", default="text", help="Path to the directory to store text output."
    )
    ...
    process_wav_files(model, args.wav_directory, args.output_directory)
